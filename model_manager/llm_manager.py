"""
LLM Manager for multiple providers: Ollama (local), OpenAI API, Anthropic API
Handles API calls, retries, rate limiting, and fallback logic with proper logging
"""
...
class LLMManager:
    ...
    Handles Ollama (local), OpenAI API, and Anthropic API
    """
    ...
    def __init__(self, 
        ...
        self._rate_limit_refill_rate = 1.0  # tokens per second
    ...
    def _check_rate_limit(self) -> bool:
        ...
        self._rate_limit_tokens = min(
            10,
            self._rate_limit_tokens + time_passed * self._rate_limit_refill_rate
        )
    ...
    def complete(self,
        ...
        try:
            if provider == LLMProvider.OLLAMA:
                return self._complete_ollama(prompt, model, temperature, max_tokens, system_prompt, json_mode)
            elif provider == LLMProvider.OPENAI:
                return self._complete_openai(prompt, model, temperature, max_tokens, system_prompt, json_mode)
            elif provider == LLMProvider.ANTHROPIC:
                return self._complete_anthropic(prompt, model, temperature, max_tokens, system_prompt)
            else:
    ...
    def _complete_openai(self,
        ...
        log_info("OpenAI completion successful",
                model=model,
                tokens_used=tokens_used,
                latency_seconds=round(latency, 3))
    ...
    def _complete_anthropic(self,
        ...
        log_info("Calling Anthropic API",
                model=model)
        ...
        log_info("Anthropic completion successful",
                model=model,
                tokens_used=tokens_used,
                latency_seconds=round(latency, 3))
    ...
    def generate_structured_json(self,
        ...
        """
        system_prompt = (
            f"You are a helpful assistant that returns valid JSON.\n"
            f"Expected schema:\n{schema_description}\n\n"
    ...
    def batch_complete(self,
        ...
        for i, prompt in enumerate(prompts):
            ...
            response = self.complete(prompt=prompt, provider=provider, **kwargs)
    ...
    def get_provider_info(self, provider: LLMProvider) -> Dict[str, Any]:
        ...
        if provider == LLMProvider.OLLAMA:
            ...
            if info["available"]:
                info["models"] = ["gpt-3.5-turbo", "gpt-4", "gpt-4-turbo-preview"]
